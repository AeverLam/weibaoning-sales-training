...
TOPIC_KEYWORDS = {
    ...
    "机制与循证": ["机制", "作用", "原理", "垂体", "FSH", "LH", "雌激素", "内膜萎缩", "病灶", "微球", "PLGA", "突释", "缓释", "Cmax", "血药浓度", "临床试验", "研究", "证据", "指南", "共识", "推荐", "随机", "对照", "抑制", "受体", "分泌"],
...
- 用户说"微球技术实现缓释"→ "微球是怎么控制药物缓慢释放的？降解过程是怎样的？"
- 用户说"疼痛缓解效果好"→ "您说的效果好，具体疼痛评分降低了多少？多少患者能达到明显缓解？"
...
4. 可以结合用户说的零星内容，从具体角度切入"""
            ...
            strategy = "ACCEPTABLE"
            strategy_instruction = f"""【策略：深入追问（用户回答质量一般）】
用户说了「{user_message}」，有一定内容但不完整或不深入。
...
1. **必须基于用户回答的具体内容追问**——从用户提到的某个点深入问下去
2. **禁止横向跳转**：不要跳到用户没提到的新话题
3. **追问要有深度**：问"具体数据""作用机理""临床证据"，深入用户提到的点

正确示例（顺着深入）：
- 用户提到"抑制垂体"→ "抑制垂体后，雌激素降到什么水平？这个水平能维持多久？"
- 用户提到"微球结构"→ "您说的微球结构，具体是怎么控制药物释放速度的？"

错误示例（横向跳转）：
- 用户提到"抑制垂体"→ "那安全性怎么样？"（跳到了安全性，不是深入机制）

4. 追问要承接用户说的具体内容，深入挖掘"""
...
【角色边界 - 严格禁止】
- 你是医生，正在被医药代表拜访，你是"被问"的一方
- 你只能：回答代表的问题、提出对产品的疑虑、表达临床需求、要求代表解释产品
- 你禁止：询问代表的"诊疗经验"、"临床工作"、"患者反馈"——这些是代表问你，不是你问代表
- 你的问题应该是：对维宝宁的疑问、对数据的质疑、对适用场景的困惑
- 错误示例（禁止）："请问您在日常诊疗中...""您遇到过哪些患者...""您怎么处理..."
- 正确示例："维宝宁在痛经缓解上有什么数据？""这个和达菲林有什么区别？""术后患者能用吗？"
