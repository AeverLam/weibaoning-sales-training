...
# -*- coding: utf-8 -*-
"""
维宝宁销售话术对练 - AI智能版飞书机器人 (文件持久化版本)
基于 render_app_ai.py，仅修改存储方式为文件持久化
保持原有对话质量和功能不变
"""
import json
import os
...
# 用户会话映射文件（user_id -> session_id）
SESSIONS_FILE = os.path.join(os.path.dirname(__file__), 'data', 'user_sessions.json')
...
💡 尽量使用专业术语和产品数据，AI会评估你的回答质量！"""
    ...
    # 选择医生角色
    if text in ROLES and not session_id:
...
@app.route('/')
def index():
    return jsonify({'status': 'ok', 'version': 'ai-persistent-v2.2', 'features': ['AI智能对话', '实时评估', '个性化反馈', '会话持久化', '5种医生角色']})
